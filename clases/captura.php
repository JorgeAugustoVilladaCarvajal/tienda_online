...
if(is_array($datos)){
    ...
    // CANDADO DE SEGURIDAD: Verifica si la transacción ya existe en la base de datos
    $sql_verificar = $con->prepare("SELECT COUNT(id) FROM compra WHERE id_transaccion = ?");
    $sql_verificar->execute([$id_transaccion]);
    $existe = $sql_verificar->fetchColumn();

    // Solo se ejecuta si la compra NO ha sido registrada previamente
    if($existe == 0){
